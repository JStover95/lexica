...
import { IDictionaryEntry, IPhrase, IReadState } from "../../utils/interfaces";
import { scrollToMiddle } from "../../utils/utils";

type Action =
  | { type: "INIT_BLOCKS"; text: string }
  ...
  | { type: "SET_DICTIONARY_ENTRIES"; previousText: string; index: number; entries: IDictionaryEntry[] };


const reducer = (state: IReadState, action: Action) => {
  switch (action.type) {
    case "INIT_BLOCKS": {
      const { text } = action;
